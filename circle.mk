# Circle OS - Top-level vendor makefile
# Includes all Circle-specific makefiles into the build.

# Core configuration (properties, product packages)
include vendor/circle/config/common.mk

# Low-level security hardening (hardened_malloc when built, hardened defaults).
# Board-level kernel mitigations are in security/board-hardening.mk, included
# from each device's BoardConfig.mk.
$(call inherit-product, vendor/circle/security/hardening.mk)

# /data/circle dirs are created by this init script on first boot.
PRODUCT_COPY_FILES += \
    vendor/circle/etc/init/circle_init.rc:$(TARGET_COPY_OUT_SYSTEM)/etc/init/circle_init.rc

# microG (GmsCore + FDroid) — provides Play Services replacement.
include vendor/circle/microg/microg.mk

# Circle OS boot animation (shongololo logo)
PRODUCT_COPY_FILES += \
    vendor/circle/bootanimation/bootanimation.zip:$(TARGET_COPY_OUT_SYSTEM)/media/bootanimation.zip
